...
export function resolveBeopjungriFromLooseAddressLine(
  ...
  raw: string,
): { rows: RegionNameInfo[]; codes: string[] } {
  const tokens = tokenizeLooseAddressLine(raw);
  if (tokens.length === 0) {
    return { rows: [], codes: [] };
  }
  ...
  const rowMatchesAllTokens = (r: RegionNameInfo): boolean => {
 ...
 */
export function commonTierCodesFromLooseRows(rows: readonly RegionNameInfo[]): {
