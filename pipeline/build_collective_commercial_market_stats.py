#!/usr/bin/env python3
"""
collective_commercial_transactions → market_stats + collective_commercial_region_annual_stats.

집합상가·집합공장 — Layer 3 market_stats 도메인 공백 메움. 모두 collective_stats DB 내 단일 연결.
region grain = `region_canonical.canonical_select_expr` (D-028). 원장 beopjungri 불변.

기존 collective_commercial_cluster_annual_stats(cluster_key 그레인, 모달 추세용)는 무변경 — 이 스크립트는
region_code 그레인의 별도 mart(collective_commercial_region_annual_stats)만 채운다.

설계: docs/REGIONAL_PROFILE_ARCHITECTURE.md §12 (D-027)
DDL: db/043_collective_commercial_region_annual_stats.sql

예)
  cd pipeline
  python build_collective_commercial_market_stats.py --windows 3,5
"""
...
ASSET_DOMAINS: dict[str, str] = {
    "collective_shop": "collective_shop_market",
...
def _distinct_sido(conn) -> list[str]:
    rows = conn.execute(
        text(
            "SELECT DISTINCT sido_code FROM collective_commercial_transactions "
...
def upsert_cc_region_annual(records: list[dict], engine) -> None:
    if not records:
        return
    sql = text(
        """
        INSERT INTO collective_commercial_region_annual_stats (
...
def ensure_cc_annual_table(engine) -> None:
    ddl_path = REPO / "db" / "043_collective_commercial_region_annual_stats.sql"
...
def build_rolling(engine, *, as_of: date, windows: list[int], sido_filter: str | None, batch_id: str) -> int:
    total = 0
    ...
    for wy in windows:
        ...
        log.info("cc market window=%sy period=%s..%s", wy, ps, pe)
        for sc in tqdm(sidos, desc=f"cc-mkt-w{wy}"):
...
def build_annual(engine, *, sido_filter: str | None, batch_id: str) -> int:
    ensure_cc_annual_table(engine)
    total = 0
    with engine.connect() as conn:
        sidos = [sido_filter] if sido_filter else _distinct_sido(conn)

    for sc in tqdm(sidos, desc="cc-annual"):
        params = {"sido": sc}
        with engine.connect() as conn:
            rows = conn.execute(
        ...
        records = _rollup_annual([dict(r) for r in rows], batch_id=batch_id)
        upsert_cc_region_annual(records, engine)
        total += len(records)
    return total
...
def main() -> None:
    ...
    log.info("collective_commercial_transactions rows=%s as_of=%s", tx_n, as_of)
    if not tx_n:
        raise SystemExit("collective_commercial_transactions empty")

    if not args.annual_only:
        n = build_rolling(engine, as_of=as_of, windows=windows, sido_filter=sido, batch_id=batch_id)
        log.info("market_stats(collective_commercial domains) upserted ~%s rows", n)

    if not args.rolling_only:
        n = build_annual(engine, sido_filter=sido, batch_id=batch_id)
        log.info("collective_commercial_region_annual_stats upserted ~%s rows", n)
