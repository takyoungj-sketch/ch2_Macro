...
export const RESIDENTIAL_FLOOR_INDEX_HELP: AnalysisExplain = {
  ...
  formula:
    "ln(㎡당단가) = β₀ + ln(전용면적) + 연식 + (동·면적·권리 탭 시 상대층 통제) + (코호트 시 단지 FE) + Σ γ_g·D_g",
  index_rule:
  ...
  controls: ["ln(전용면적)", "연식", "상대 층구간(비층 탭)", "단지 고정효과(코호트)"],
  interpretation: [
    "지수는 「비슷한 전용면적·연식」 조건에서의 층·동·면적·권리 간 상대 수준입니다.",
    "100%보다 낮을수록 기준 대비 ㎡당 단가가 낮은 패턴입니다.",
    "95% CI·p-value는 계수 불확실성·유의성 참고용입니다.",
  ],
  limitations: [
