...
export const RESIDENTIAL_FLOOR_INDEX_HELP: AnalysisExplain = {
  ...
  summary:
    "같은 단지(또는 코호트) 실거래의 ln(㎡당단가)를 회귀합니다. 전용면적·연식·거래시점(반기)을 맞춘 뒤, 층·동·면적형·권리 한 차원만 상대 %로 봅니다. 아파트는 단지별 최고층 대비 1·저·중·고·최상, 연립·다세대는 1층·중간·최상입니다. 회귀 분석 탭(금액 OLS)과 spec·숫자가 다릅니다.",
  formula:
  ...
  floor_groups: [
    "아파트: 그 단지 최고층 대비 1층 · 저층부(30% 이하) · 중층부(30~70%) · 고층부(70% 초과) · 최상층. 여러 단지는 단지마다 나눕니다.",
    "연립·다세대: 1층 · 중간층 · 최상층(그 건물 거래 최고층). 개별 층·절대 구간으로 바꿀 수 있습니다.",
...
export const CLUSTER_FLOOR_INDEX_HELP: AnalysisExplain = {
  ...
  summary:
    "같은 도로명 cluster 실거래의 ln(㎡당단가)를 회귀합니다. 상가 층은 지하·1·2·저·중·고·초고층, 공장·창고는 지하·1·2·3층 이상입니다. 상가 면적은 목록과 같은 35·50·100·300㎡ 구간이고, 공장은 100/300/1000㎡입니다. 1층 또는 중앙 면적구간=100%. 회귀 탭과 숫자가 다릅니다.",
  formula:
  ...
  floor_groups: [
    ...
    "면적별(상가): 35㎡ 미만 · 35~50 · 50~100 · 100~300 · 300㎡ 이상. 기준=표본 중앙값 칸.",
...
/** 지역회귀 모달 — 쌍둥이지역 탭 */
export const APT_TWIN_REGION_HELP: AnalysisExplain = {
  spec_id: "apt_twin_region_tab_v1",
  spec_version: "2",
  title: "쌍둥이 지역을 고르는 방법",
  summary:
    "이 목록은 기준 읍면동의 아파트 단지 구성과 아파트 가격분포가 가까운 전국 읍면동입니다. " +
    "가격의 정답을 고르는 순위가 아닙니다.",
  interpretation: [
    "후보가 되려면 최근 3년 창에서 거래가 5건 이상이고, 연식·최고층·세대수·세대당 주차·개별공시지가가 있는 아파트 단지가 20곳 이상이어야 합니다.",
    "먼저 단지 수, 거래 수, 연식·최고층·세대수·세대당 주차의 중앙값과 사분위 범위, 개별공시지가의 중앙값과 사분위 범위로 전국에서 20곳을 남깁니다. 항목마다 가중치를 두지 않고, 같은 척도로 표준화한 거리를 씁니다.",
    "그 20곳을 아파트 단지 ㎡당 가격의 25백분위, 중앙값, 75백분위, 평균, 변동계수로 다시 줄여 1위부터 5위까지 보여 줍니다. 가격도 최근 3년 창의 단지 중앙값 분포입니다.",
    "순위는 아파트만 사용합니다. 오피스텔과 연립·다세대는 거리를 계산하지 않습니다.",
    "통합회귀는 기준 읍면동과 체크한 지역의 단지를 한 식에 합칩니다. 지역회귀 탭에서 켠 유형이 있으면, 고른 지역의 그 유형 단지도 같은 식에 들어갑니다.",
    "지역 더미는 기본으로 꺼져 있습니다. 켜면 연식·층·세대수·주차·공시지가의 기울기는 지역이 공유하고, 절편만 지역마다 나뉩니다. 기준 절편은 이 읍면동이고, 가상단지 예측도 그 절편을 씁니다. 목록에서 단지를 고르면 그 단지가 있는 지역의 절편으로 예측합니다.",
    "변수, 모형, 최소 거래수, 통계 창은 지역회귀 탭과 같습니다. 순위만 3년 창으로 고정합니다.",
    "이 식은 미래 가격의 정답을 보장하지 않습니다. 고른 지역을 합쳐 돌려 보는 탐색이고, 받아들일지는 사용자가 정합니다.",
  ],
  limitations: [
    "지역을 더할수록 오차가 줄어든다는 뜻은 아닙니다. 몇 곳을 고를지는 사용자가 정합니다.",
    "단지가 충분한 지역에서 그 지역만의 식은 지역회귀 탭에 있습니다.",
    "개별공시지가는 적재된 최신 값을 씁니다.",
    "인접 읍면동보다 가깝다거나, 단독 회귀보다 정확하다는 순위가 아닙니다.",
  ],
  interpretation_hints: [],
  presets: [
    {
      id: "why_apartment_only",
      question: "오피스텔을 켜 두었는데 순위에는 왜 안 들어가나요?",
      answer:
        "쌍둥이 지역은 기준 읍면동의 아파트 재고 구성과 아파트 가격분포로 골랐습니다. " +
        "오피스텔과 연립·다세대는 순위에 쓰지 않았습니다. 통합회귀를 실행하면 고른 지역의 그 유형 단지도 같은 식에 들어갑니다.",
    },
  ],
  controls: [],
  floor_groups: [],
...
export const RESIDENTIAL_REGRESSION_HELP: AnalysisExplain = {
  ...
  summary:
    ...
    "기본은 선형(만원), 로그는 % 변화 옵션. 거래시점(반기)은 선택 항목이고 기본으로 켜 둡니다. 층 %는 효용지수 탭입니다.",
  formula: "금액(만원) = β₀ + Σ β_k·X_k  (선형 OLS · 기본)",
  reference: "범주형 변수는 drop_first 기준 범주 대비",
  floor_groups: [
    "아파트 relative: 단지별 최고층 대비 1·저·중·고·최상",
    "연립 rowhouse: 1층·중간·최상",
    "dummy: 개별 층 더미",
    "grouped: 1–5 / 6–15 / 16+",
    "linear: 층 선형",
  ],
  controls: ["전용면적", "연식", "층", "동(아파트·연립)", "권리(분양권)", "코호트: 세대수·주차·공시지가·구조·유형(FE 대체)"],
  interpretation: [
