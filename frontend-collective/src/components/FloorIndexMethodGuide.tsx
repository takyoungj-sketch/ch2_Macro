...
export function floorIndexGuideText({
  ...
  if (dimension === "floor") {
    const groups = isFactory
      ? "지하 · 1층 · 2층 · 3층 이상입니다. 상가의 고층·초고층 구간을 쓰지 않습니다."
      : isCluster
        ? "지하심층(B2 이하) · 지하1층 · 1층 · 2층 · 저층(3–4) · 중층(5–9) · 고층(10–19) · 초고층(20+). 지하를 주거 중층부로 넣지 않습니다."
        : floorMode === "dummy"
          ? "거래가 있는 개별 층마다 칸을 만듭니다."
          : floorMode === "grouped"
            ? "절대 층 구간 1–5층 · 6–15층 · 16층+입니다."
            : floorMode === "rowhouse"
              ? "1층 · 중간층(2층~최고 바로 아래) · 최상층입니다. 최상층은 그 건물의 거래 최고층입니다."
              : "그 단지 최고층 대비 1층 · 저층부 · 중층부 · 고층부 · 최상층입니다. 단지가 여러 곳이면 단지마다 나눕니다.";
    return {
      title: "층별 지수",
      body:
        `같은 ${isCluster ? "도로(cluster)" : "단지"} 실거래의 ㎡당 단가(로그)를 회귀합니다. ${groups} ` +
        `${isCluster ? "연면적" : "전용면적"}·연식·거래시점(반기)을 맞춘 뒤, ${ref}=100으로 층만 비교합니다. ` +
        "p는 거래가 가장 많은 칸 대비이고, 그 칸의 p는 없습니다. " +
  ...
  if (dimension === "area") {
    const buckets = isFactory
      ...
      : isCluster
        ? "연면적 구간은 거래 목록과 같습니다. 35㎡ 미만 · 35~50㎡ · 50~100㎡ · 100~300㎡ · 300㎡ 이상입니다."
        : "전용면적을 30㎡로 반올림한 면적형입니다.";
    return {
      title: "면적별 지수",
      body:
