...
export default function AnalysisHelpPanel({
  ...
  return (
    <div className={clsx("relative", className)}>
      ...
      {open && (
        <div className="mt-2 rounded-lg border border-indigo-100 bg-indigo-50/30 p-3 space-y-3 max-h-[min(420px,55vh)] overflow-y-auto">
              ...
              <div className="space-y-1">
                {explain.presets.map((p) => (
                  <div key={p.id} className="rounded border border-slate-100 bg-white/70 overflow-hidden">
                    ...
                    {activePreset === p.id && (
                      <p className="px-2 pb-2 text-[11px] text-slate-600 border-t border-slate-50">{p.answer}</p>
                    )}
                  </div>
                ))}
              </div>
          ...
          <p className="text-[9px] text-slate-400 pt-1 border-t border-slate-100">
            차후 AI Q&A는 이 spec·수치(JSON)만 근거로 답변하도록 연동할 예정입니다.
          </p>
        </div>
      )}
    </div>
