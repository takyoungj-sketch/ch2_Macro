...
function buildCols(assetType: AssetType, showBuilding: boolean): ColDef[] {
  const cols: ColDef[] = [];
  if (showBuilding) {
    cols.push({ key: "building", label: "단지", filterType: "select" });
  }
  cols.push(
    ...
    { key: "buyer_type", label: "매수", filterType: "select" },
    { key: "seller_type", label: "매도", filterType: "select" },
    { key: "deal_type", label: "거래유형", filterType: "select" },
  );
  return cols;
}
...
export default function CollectiveTransactionTable({
  items,
  assetType,
  showBuilding = false,
  truncated,
  ...
  const [selectFilters, setSelectFilters] = useState<Partial<Record<CollectiveTxSortKey, Set<string>>>>({});
  const [sortKey, setSortKey] = useState<CollectiveTxSortKey>("contract_date");
  const [sortDir, setSortDir] = useState<CollectiveTxSortDir>("desc");
  const [page, setPage] = useState(1);
  const [openFilterCol, setOpenFilterCol] = useState<CollectiveTxSortKey | null>(null);
  const dropdownContainerRefs = useRef<Partial<Record<CollectiveTxSortKey, HTMLDivElement | null>>>({});
  ...
  const distinctValues = useMemo(() => {
    const result: Partial<Record<CollectiveTxSortKey, string[]>> = {};
    for (const col of SELECT_COLS) {
      const vals = new Set<string>();
      for (const item of items) {
        vals.add(getSelectDisplayValue(item, col.key, assetType));
      }
  ...
  const processed = useMemo(() => {
    ...
    for (const col of SELECT_COLS) {
      ...
      rows = rows.filter((r) => sel.has(getSelectDisplayValue(r, col.key, assetType)));
    }

    rows.sort((a, b) =>
      compareValues(collectiveTxSortValue(a, sortKey, assetType), collectiveTxSortValue(b, sortKey, assetType), sortDir),
    );
    return rows;
  }, [items, SELECT_COLS, selectFilters, sortKey, sortDir, assetType]);
  ...
  const handleSort = (key: CollectiveTxSortKey) => {
    setPage(1);
    if (sortKey === key) {
      ...
      setSortKey(key);
      setSortDir(
        key === "contract_date" || key === "price" || key === "unit_price" || key === "exclusive_area"
          ? "desc"
          : "asc",
      );
    }
  ...
  const isFilterActive = (key: CollectiveTxSortKey) => selectFilters[key] !== undefined;

  const selectFilterLabel = (key: CollectiveTxSortKey) => {
                ...
                <tr key={t.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/40">
                  {showBuilding && (
                    <td className="border border-slate-200 dark:border-slate-600 px-2 py-1 whitespace-nowrap">
                      {formatCollectiveTxCell(t.display_name)}
                    </td>
                  )}
                  <td className="border border-slate-200 dark:border-slate-600 px-2 py-1 tabular-nums whitespace-nowrap">
                    {formatCollectiveTxContractDate(t)}
                  </td>
                  <td className="border border-slate-200 dark:border-slate-600 px-2 py-1 whitespace-nowrap">
                    {collectiveTxDongCell(t, assetType)}
                  </td>
                  ...
                  <td className="border border-slate-200 dark:border-slate-600 px-2 py-1 whitespace-nowrap">
                    {formatCollectiveTxCell(t.buyer_type)}
                  </td>
                  <td className="border border-slate-200 dark:border-slate-600 px-2 py-1 whitespace-nowrap">
