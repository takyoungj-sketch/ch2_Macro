...
export function paidUsesCustomAreaSpan(base: PaidAnalysisRequest): boolean {
  const lo = base.area_sqm_min;
  const hi = base.area_sqm_max;
  return (
    (lo != null && Number.isFinite(lo)) || (hi != null && Number.isFinite(hi))
  );
}
