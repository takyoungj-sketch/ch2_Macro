...
import {
  ...
  calendarYearFilterCaption,
  filteredAnalysisActionLabel,
  rollingStatsCaption,
...
export default function PaidFilterTable() {
  ...
  const yearOptions = useMemo(
    () => [...getPaidYearButtonYears(freeStatsWindowYears)],
    [freeStatsWindowYears],
  );
  const selectedYears = paidRequest.years ?? [];
  const selectedDealTypes = DEAL_TYPES.filter(
    (dt) => !paidDealTypeExcluded.includes(dt),
  );
  ...
  const runFiltered = () => {
    ...
    if (resolvedRegionCodes.length === 0) {
      setFilterError(
        `먼저 지역을 입력하고 ${basicStatsActionLabel("paid")}(${rollingStatsCaption(freeStatsWindowYears)})까지 반영해 주세요.`,
      );
      return;
    }
    ...
    if (selectedDealTypes.length === 0) {
      setFilterError("거래유형을 하나 이상 선택해 주세요.");
      return;
    }
    if (customAreaSpan) {
        ...
        <tbody>
          ...
          <tr className="border-b border-slate-100 align-top">
            ...
            <td className="px-2 py-2">
              <div className="flex flex-wrap gap-1">
                {DEAL_TYPES.map((dt) => {
                  const on = !paidDealTypeExcluded.includes(dt);
                  return (
                    <button
                      key={dt}
                      type="button"
          ...
          <tr className="border-t border-slate-200 bg-slate-50/80">
            <td colSpan={2} className="p-3">
              ...
              <AnalysisActionButton
                ...
                caption={calendarYearFilterCaption}
                onClick={runFiltered}
                variant="filter"
