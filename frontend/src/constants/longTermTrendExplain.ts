...
export function buildLongTermTrendExplain(ctx: LongTermExplainContext): AnalysisExplain {
  ...
  const hints: string[] = [
    ...
    ctx.metric === "mean" && (ctx.seriesCount ?? ctx.targets.length) >= 2
      ? `지역 단위: ${levelText} — 지역별 선 / 통합선은 아래 별도 칸(거래수 가중평균 Σn·평균/Σn)`
  ...
  return {
    ...
    limitations: [
      ...
      "기본통계 보기(롤링 3·5년)와 기간 축이 다릅니다 — 롤링 탭은 창 구간, 장기 추세는 만년력 연도입니다.",
    ],
    interpretation_hints: hints,
    presets: [
      {
        id: "vs-filter",
        question: "필터분석 「선택 연도」·기본통계 「롤링 구간」과 숫자가 다른 이유는?",
        answer:
          "선택 연도·롤링 구간 탭은 도로·면적·이상치 등 필터가 반영된 live 집계입니다. " +
