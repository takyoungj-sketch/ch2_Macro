...
echo "==> frontend profile build"
cd "$REPO_ROOT/frontend-profile"
if [[ ! -f .env ]] && [[ -f "$REPO_ROOT/deploy/templates/frontend-profile.env.production.example" ]]; then
  echo "WARN: frontend-profile/.env missing — copy template and set VITE_API_TOKEN before build"
fi
npm ci
npm run build
...
echo
echo "OK: redeploy complete (gateway /, land /land/, built /built/, collective /collective/, profile /profile/)"
