#!/usr/bin/env python3
"""
복합부동산 월간 cycle — 2607업데이트 CSV(12개월) purge + ingest + scope mart.
...
순서:
  1) 계약연월 구간 purge (built_transactions)
  2) import_molit (--paths-file, region_codes refresh)
  3) build_scope_stats (--as-of cycle 매핑)
  4) 스냅샷 JSON
"""
...
def main() -> None:
    ...
    p = argparse.ArgumentParser(description="복합부동산 월간 CSV cycle (purge → ingest → mart)")
