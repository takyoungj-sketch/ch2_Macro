#!/usr/bin/env python3
"""
국토교통부 실거래가 CSV 수집 (연립·다세대 매매).

`download_molit_officetel_csv.py` 와 동일 Selenium 흐름:
  rt.molit.go.kr → 연립다세대 탭 → 계약일 · 시도 선택 → CSV 다운 ...

기본 출력: `<repo>/원본/연립다세대/<시도>_연립다세대_매매_<연도>.csv`

예)
  py scripts/monthly/download_molit_rowhouse_csv.py --limit-regions 1 --years 2021
  py scripts/monthly/download_molit_rowhouse_csv.py --start-year 2021 --end-year 2025
"""
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Molit 실거래 연립·다세대(매매) CSV 일괄 다운로드")
    p.add_argument(
        "--output-dir",
        type=str,
        default="",
        help="저장 폴더. 미지정 시 <repo>/원본/연립다세대",
    )
...
def main() -> None:
    ...
    if args.output_dir.strip():
        ...
        download_dir = (REPO_ROOT / "원본" / "연립다세대").resolve()
                ...
                try:
                    tab = wait.until(
                        EC.element_to_be_clickable(
                            (By.ID, f"xlsTab{ROWHOUSE_PROPERTY_TYPE}")
                        )
                    )
