#!/usr/bin/env python3
"""
국토교통부 실거래가 CSV 수집 (아파트 매매 · 2010~2020 등 장기 backfill용).

기본 출력: `<repo>/raw/아파트_2010_2020/<시도>_아파트_매매_<연도>.csv`

수집 코어: molit_csv_download_core (검증·안정 대기). docs/MOLIT_CSV_COLLECTOR_WARNINGS.md
"""
...
from apartment_historical_csv_status import write_manifest  # noqa: E402
from molit_csv_download_core import CollectSpec, run_molit_csv_collect  # noqa: E402

DEFAULT_OUTPUT_DIR = REPO_ROOT / "raw" / "아파트_2010_2020"
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Molit 아파트 CSV backfill")
    p.add_argument("--output-dir", default="", help=f"기본 {DEFAULT_OUTPUT_DIR}")
...
def main() -> None:
    ...
    # 연도 목록이 연속이 아닐 수 있어 regions×years 루프는 core가 range로 처리;
    # years 비연속이면 --years 로 여러 번 실행 권장
    if years != list(range(min(years), max(years) + 1)):
        print("비연속 --years 는 연도별로 스크립트를 나눠 실행하세요.")
        sys.exit(1)
