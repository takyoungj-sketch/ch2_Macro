#!/usr/bin/env python3
"""
국토교통부 실거래가 CSV 수집 (분양입주권 매매).

`참고/0.수집.ipynb` xlsTab5 와 동일.

기본 출력: `<repo>/원본/분양입주권/<시도>_분양입주권_매매_<연도>.csv`

예)
  py scripts/monthly/download_molit_presale_csv.py --limit-regions 1 --years 2023
  py scripts/monthly/download_molit_presale_csv.py --start-year 2021 --end-year 2025
"""
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Molit 실거래 분양입주권(매매) CSV 일괄 다운로드")
    p.add_argument(
        "--output-dir",
        type=str,
        default="",
        help="저장 폴더. 미지정 시 <repo>/원본/분양입주권",
    )
...
def main() -> None:
    ...
    if args.output_dir.strip():
        ...
        download_dir = (REPO_ROOT / "원본" / "분양입주권").resolve()
                ...
                try:
                    tab = wait.until(
                        EC.element_to_be_clickable(
                            (By.ID, f"xlsTab{PRESALE_PROPERTY_TYPE}")
                        )
                    )
