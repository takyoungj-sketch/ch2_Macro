...
> **작성:** 2026-06-06 · **상태:** **2026-08-22 부분 대체됨**
> **원칙:** 지번을 100% 확정하기 위한 매칭이 아니라, **구조·층수·용도·승인일** 등 건물 특성을 **확률적으로** 보강한다.
>
> **대체 안내.** 이 문서의 전제 두 개가 이후 실측으로 바뀌었다.
> ① 「지번 완전 복원 불가」(§2.2·§12) — 연면적 완전일치를 *탈락 조건*이 아니라 *필지 특정 수단*으로 쓰면 충북 81.5%·서울 68.1%가 확정된다. SSOT는 [`BUILT_DATA_ENRICHMENT.md`](BUILT_DATA_ENRICHMENT.md)·[`D-047`](lab/decisions/D-047.json).
> ② 「DB 위치 미결정」(§10) — 필지(PNU) 기반 축약대장을 **로컬 빌드 타임 자산**으로 두는 설계 제안이 [`PARCEL_MASTER_DESIGN.md`](PARCEL_MASTER_DESIGN.md) 에 있다. §4(1순위 적재)·§7(집합 building_key)·§8(DB 매핑)은 그 문서를 따른다.
> 여전히 유효한 것: 구조 5종 축약(§5.2), 면적 hard filter 금지(§2.3), 전유부·소유자 미포함.
...
> **2026-08-20 갱신 — 아래 2.8%는 재현되지 않았다.** 청주 파일럿에서 연면적 **완전일치(±0.01㎡)** 를
> *탈락 조건*이 아니라 *필지 특정 수단*으로 쓰면 단독다가구 **76.4%**, 상업업무 78.7%가 지번까지 확정된다
> (도로명·사용승인연도 독립 검증 ~99%). 마스킹 지번 복원의 SSOT는
> [`BUILT_MASKED_ADDRESS_RECOVERY.md`](BUILT_MASKED_ADDRESS_RECOVERY.md)다.
> 여전히 유효한 것: 전 거래 100% 복원 불가(18.7% 실패), 대지·건축·연면적 동시 exact 불가.

- `gross_area`(실거래) vs 대장 `연면적`/`건축면적`: **hard filter 금지**, **후보 순위·신뢰도** 보조만
- 충북 복합 실측: `beopjungri + lot_prefix`만 → 후보 **88%**, 구조 1종 확정 **11%**
- 여기에 연면적 **0.1㎡ exact** 추가 → 후보 **2.8%**로 급감 (97% 탈락) ← *2026-08-20 실측과 불일치, 재현 불가*
- **상대 오차(±5~10%) + 최근접 1건**은 scoring용으로 검토 가능하나, “확정” tier로 승격하려면 2등과 격차 검증 필요
- **대지면적은 필수 조건 금지** — 표제부 보유율 42.6%(총괄 보완 후 필지 56.7%)라 걸면 25%p 손실
