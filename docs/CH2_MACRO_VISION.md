...
토지·복합·집합에서 **통계·회귀·예측·한계 노출·AI**를 같은 언어로 제공한다.  
**모형 추천**은 목적별 후보 제시이며, **Twin은 검증 전제 보조 pool**이다.  
Twin이 Local 대비 **검증 지표상 개선**을 보일 때만 제품 기본으로 승격한다.  
상세: [CH2_MIDCHECK_IMPROVEMENT_PLAN.md](./CH2_MIDCHECK_IMPROVEMENT_PLAN.md) · [CH2_CONSTITUTION.md](./CH2_CONSTITUTION.md) §5-1.
...
| 회귀 | 통제 후 패턴 |
| 예측 | 한 점 예측 + 구간·한계 |
| 모형 추천 | 설명형 / 예측형 후보 (정답 식 아님) |
| Twin | 유사 지역 pool → **Validation으로 채택 여부** |
| AI | Facts 해설 (판사 아님) |
