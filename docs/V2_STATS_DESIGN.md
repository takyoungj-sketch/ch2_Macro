...
- **`window_years`**: 소수 연 단위 **정수 1~7** — “**직전 월 말일**”을 끝으로 두고, 그날부터 **달력으로 N년 전**까지 거슬러 올라간 구간(서는 **다음 날**)으로 본다.
- **제품 정책 (2026-08)**  
  - **무료·공통 UI**: `3`, `5`(기본), `7` 노출. 상세: [`ROLLING_WINDOW_7Y_PLAN.md`](ROLLING_WINDOW_7Y_PLAN.md).  
  - **유료(추후)**: `1`~`7` 세분화 여지(DDL 상한 7).
...
4. **`window_years ∈ {1,2,3,4,5,6,7}`** 각각에 대해 `period_start`/`period_end` 계산 (운영 mart는 보통 `3,5,7`만)  
