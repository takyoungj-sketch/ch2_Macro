...
**구현 주의 (2026-08-03 발견 → 2026-09-19 정책 변경, D-074):** log-scale rolling
CV-MAPE에서 test fold의 범주 조합이 train에 드물게 나타나면
`exp(pred) * duan_smearing`이 수치적으로 발산해(관측 값 대비 10^150배 등) CV-MAPE가
아주 커질 수 있다 (`selection/fit.py::_rolling_time_cv_mape`). pool처럼 여러 지역을
합친 표본에서 특히 발생하기 쉽다.

한동안 train 표본의 관측 가격 범위(×0.1~×10)로 예측을 **clip**해 이를 막았지만,
**clip은 더 이상 공식 CV-MAPE에 쓰지 않는다.** 크게 실패한 예측을 잘라내면 실패한
후보가 덜 실패한 것처럼 보여, 모델 선택 지표가 성능을 정직하게 반영하지 못한다.
경계는 계속 계산하되 판정에만 써서 `cv_extreme_rate`(경계 밖 예측 비율)·`cv_max_ape`·
`cv_median_ape`를 **안정성 진단**으로 남긴다. 발산하는 후보는 raw MAPE가 커져 순위에서
스스로 탈락하고, 진단 필드가 그 이유를 설명한다.

**CV-MAPE를 읽을 때의 단서 (2026-09-19 정리):**

- **거래 가중 평균이다.** fold별 MAPE를 평균하지 않고 모든 fold의 개별 오차를
  한 번에 모아 평균한다. 거래가 많은 연도가 지표를 더 끌어당기므로 화면의
  「fold n개」는 표본 수가 아니라 평가한 연도 수로만 읽어야 한다.
- **예측을 자르지 않는다 (D-074).** 발산하는 log·log-log 후보는 raw MAPE가 커져
  그대로 불리해진다. 대신 `cv_extreme_rate`로 얼마나 튀었는지를 따로 본다. 평균
  APE는 소수 관측에 좌우될 수 있으므로 `cv_median_ape`를 함께 기록하고, 중위 순위와
  평균 순위가 크게 엇갈리면 공식 지표 변경을 재검토한다.
- **설계행렬은 전체 표본에서 한 번 만들고 연도로만 나눈다.** 학습 fold에 없는
  범주의 더미는 전부 0인 열이 되므로, fold마다 `_estimable_columns`로 추정 불가
  열을 빼고 적합한다 (뺀 뒤에도 rank가 모자라면 그 fold를 건너뛴다). 2026-09-19
  이전에는 축퇴된 pinv 해를 그대로 썼다.
- **마지막 연도는 선택에서 완전히 분리한다 (D-074).** 모든 후보가 탐색 CV(마지막
  연도 제외)와 확인 CV(마지막 연도)를 함께 들고 나온다. 랭킹은 탐색 CV로만 하고,
  `stage1.primary_confirm_cv_mape`는 1위 후보가 이미 가진 확인 CV를 그대로 쓴다.
  2026-09-19 이전에는 랭킹 CV가 마지막 연도를 테스트 fold로 포함한 뒤 같은 연도로
  「확인」해서 확인이 확인이 아니었다. 탐색 CV는 여전히 수백 조합의 최소값이라
  낙관적이므로 두 값의 격차를 안정성으로 읽는다.
- **얇은 표본은 탐색 CV가 없을 수 있다.** 마지막 연도를 떼면 학습 fold가 fold 적합
  최소 건수를 못 채운다. 이때 **AIC로 대체 판정하지 않는다** — 로그우도가 n에 따라
  커져 표본이 다른 모형끼리 AIC는 비교할 수 없다. Twin 판정은 보류하고 Local을
  유지한다 (`selection/pooling.py::_primary_value`).
- **Best Subset 상한은 511이다.** 제품 후보 블록이 최대 9개(2⁹−1 = 511)이므로
  잘림 없이 전수 평가한다. 조합은 블록 수 오름차순으로 열거해, 상한에 걸리는
  경우에도 특정 블록(`region_leaf` 등)이 통째로 빠지지 않는다.
