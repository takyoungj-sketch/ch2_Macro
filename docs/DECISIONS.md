...
- **등급 입력:** 탐색·확인 중 **나쁜 쪽.** 확인이 좋아도 올려 주지 않음(의도된 비대칭). 한쪽만 있으면 그 값으로.
- **중위 APE로 바꾸지 않음**(종결). 전수 31후보에서 강남구 중위 1위 = 평균 17위인 2변수 모형. 중위는 꼬리 두꺼운 희소 모형을 선호. 중위는 화면 진단만.
- **숫자가 나빠지는 것은 버그가 아니다.** 62.5% → 44.7% → 38.1%와 달라져도 회귀 아님. 완료 후 Local·Upper·Twin 전부 재실행한 값이 새 기준선. 등급 구간 재보정은 P4 후.
...
## D-075 가격시점 보정은 회귀식 밖 별도 계층 — **구현 후 보류 (제품 기본 off)**

- 회귀는 **물건 구조만**. 거래시점 → 가격지수 → 기준시점 환산은 **별도 계층**. 결과 = 회귀 추정값 × 시점 보정.
- **연도 더미를 넣지 않음.** 학습에 없는 연도는 계수 추정 불가이고, 얇은 국소 표본의 자유도를 쓴다.
- 지수 추정 단위는 **시군구.** 구조는 국소, 시간은 광역 — 상위지역 재적합·Twin과 같은 논리.
- 얇은 시군구는 시도 승격이 아니라 **empirical Bayes 수축.** 연 30건 게이트 + 잡음만큼 0으로 당김. 진행 중인 해는 기준시점으로 쓰지 않음.
- **제약:** 지수는 **holdout 이전 데이터로만** 추정. CV는 fold마다 학습 연도만으로 재추정, 평가 연도 실제가는 손대지 않음.
- **제품에 넣지 않음 (`time_adjust=False`).** 5년 창 실측에서 adj R²·MAPE 개선 근거 없음(부호 섞임, 6곳 중 2곳은 지수 자체가 없음). 그런데 가격 수준은 20~37% 이동. P4는 정확도가 아니라 **정의** 작업이라 CV로 검증이 **원리상 불가**하므로, 검증 불가능한 20%대 이동을 잡음 섞인 지수에 근거해 태우지 않음.
- **첫 측정은 창을 잘못 잡았다.** `as_of_month` 누락으로 창 필터 미적용 → 8년치. 그 위의 「효과가 크다」 판단도 틀렸음. 8년/5년 대조는 계획문서 §12.
- 재검토: 창을 늘리거나 지수 잡음을 줄인 뒤. **창 길이 논의 시 P4를 같이 올린다** — 창이 길수록 시점 미통제의 대가가 커진다(8년에서 강남 연식 계수 절반 축소).
- 카드 [`lab/decisions/D-075.json`](lab/decisions/D-075.json).

## D-076 복합 회귀 잔차 진단 — 「이 식이 어디서 틀리나」

- 사용자가 **자기 회귀식을 먼저 만드는** 구조라면 그 식이 어디서 틀렸는지 보여 주는 화면이 있어야 한다. Adj R²·MAPE는 「평균적으로 얼마나」만 말한다. **추천 기능의 대체재.**
- **초점 모형에만** 붙는다(`RegressionLevelResult.residuals`). 상위 비교는 화면이 없고 n이 수만까지 간다.
- 잔차 정의를 **MAPE와 묶었다.** 오차% = (실제 − 예측) ÷ 실제 × 100, 예측은 상단 MAPE와 같은 식(log면 Duan). 집단별 MAPE를 건수 가중평균하면 상단 MAPE가 되고 이 등식을 테스트로 고정. 이분산 검정만 log 공간.
- **편향은 중위로.** 첫 실측에서 평균 편향이 강남 −12.6%·수성 −19.0%로 나오고 거의 모든 집단이 「유의」하게 찍혔다. 모형 결함이 아니라 백분율 오차의 성질 — 작은 거래는 −355%가 가능하지만 +100%를 넘지 못해 평균이 구조적으로 음수다.
- **유의성은 「그 집단 vs 나머지」 Mann–Whitney.** 0과 비교하면 공통 치우침 때문에 전 집단이 유의해진다. 화면이 읽는 값은 `excess_bias_pct`. 10건 미만 집단은 표에서 뺀다.
- **이분산은 분위 산포 우선, BP는 보조.** 강남 BP p=0.000인데 산포 42~46% 평평, 수성 p=0.109인데 83→43% 급감 — 양방향으로 엇갈렸다. BP는 금액대가 아닌 설계행렬 전체에 반응한다.
- 영향 상위 5건은 **Cook 거리** + 그 5건 뺀 **재적합 계수 변화(표준오차 배수)**. 거래목록 모달 id 필터는 **안 넣는다** — 5앱 공유 위젯이라 패리티(D-072)에 걸린다. 인라인 표로 대체.
- **in-sample이다.** 예측력은 모형 탐색 CV-MAPE로. 이 숫자를 예측 정확도로 인용 금지.
- **§9 두 항목의 진입 조건을 만들었다** — 연식 비선형(강남 신축 +8.6%p / 20–29년 −6.9%p), 공간모형(지역 7곳 유의). 단 공간모형 전에 지역 더미·Twin을 먼저 쓴다.
- 카드 [`lab/decisions/D-076.json`](lab/decisions/D-076.json).
