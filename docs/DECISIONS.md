...
| D-026 | 2026-06-30 | **토지 지목 7대분류(`jimok_group`) 및 매트릭스 재구성**: 지목을 농경지·산림지·개발지·기반시설·수면·특수용도·기타(7그룹)로 묶어 통계 매트릭스 열(또는 기본 뷰)을 재구성한다. **원장 Master는 수정하지 않고** `land_jimok_group_map` + VIEW 파생 + **mart 재집계**로 반영. **구현·전국 mart 재구축은 `cycle_id=202607`(7월 초) 월간 업데이트와 동시**에 수행한다. 분류표·매핑·일정: [`docs/LAND_JIMOK_GROUP_DESIGN.md`](LAND_JIMOK_GROUP_DESIGN.md). 광천지·염전은 **⑥ 특수용도**; ⑦은 미매핑·미분류 잔여. |
