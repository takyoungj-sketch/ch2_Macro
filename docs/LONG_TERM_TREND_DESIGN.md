...
| 지역 표시 | **지역별 시리즈**를 **한 차트·한 모달**에 겹침. **평균 모드** 시 거래수 가중 통합선 추가; 중앙값 모드는 통합선 없음 |
...
2010~2020 backfill 시 이력 테이블 **필수** (설계). **현재(v1 배포): `region_code_history` 미적재 → §2.4 주의.**

### 2.4 v1 배포 시 해석 주의 (행정구역 — 세종 한정 아님)

| 현상 | 원인 | 사용자 해석 |
|------|------|-------------|
| 상위(시·도·시군구) 명칭이 과거 연도에도 현행 | 국토부 CSV 소급 표기 | 청원군→청원구 등 **개명만**인 경우 연속성 **상대적으로 양호** |
| 하위(동·리)에서 선 단절·공백 | 리→동 통폐합, 신설 행정동, 마스터에 옛 리 없음 | **무거래 ≠ 해당 연도 무의미**; 옛 리·면 또는 미매핑 원장 가능 |
| 현재 동 선택 시 2010~ 선 없음 | 집계 키가 현행 beopjungri/eup 코드 | **신도시·통폐합 지역**에서 흔함 (세종, 타지 소규모 개편 포함) |
| 같은 시·군·구명, 다른 시대 | 코드 유지·명칭만 변경 vs 구역 재편 | case-by-case; **선 존재만으로 지역 동일 가정 금지** |

**배포 전·v2 검토 (장기 과제):**

- `region_code_history` 조사·적재 + backfill/remap 파이프라인 (§2.3)
- 세종 등 **단일 sigungu_code(36110)** UI·매핑 특례
- `clean.py` **동-only 주소**(예: `세종… 집현동`) → beopjungri 매핑 보강
- 장기 탭: 데이터 없을 때 **「집계 불가 / 행정개편」** vs **「무거래」** UI 구분

**v1 정책:** 위 항목은 **기능 보류**, help 패널·API `disclaimer`에 주의 문구 명시 (`longTermTrendExplain.ts`, `LONG_TERM_DISCLAIMER`).
