...
> **상태:** 원장·건물 마트·목록 UI · **전환율 연구 종료 · `mean_simple` 확정 (D-040)** · **B 상권 병기(하위시장)** · **매매 조인=분석층(3안) · 구현 [`RENT_COLLECTIVE_SALE_JOIN_PLAN.md`](./RENT_COLLECTIVE_SALE_JOIN_PLAN.md)**  
...
# 매매×임대 매칭률: py pipeline/rent/report_sale_join_coverage.py
# 정확 키 맵: py pipeline/rent/build_sale_building_map.py
```
...
## 매매 창 조인 (3안, 2026-08 확정)

매매와 임대를 **원장·마트·진입에서 합치지 않는다.** 홈(`/`) 매매/임대 분기와 임대 전용 화면을 유지한다. 한 건물에서 매매·전월세를 같이 보는 것은 **분석층 조인 뷰**다.

구현 순서·매칭 층·게이트: [`RENT_COLLECTIVE_SALE_JOIN_PLAN.md`](./RENT_COLLECTIVE_SALE_JOIN_PLAN.md).
...
- 분양권·비주거 집합(`cluster_key`)·복합·상권 B를 이 조인에 넣기
- Twin 벡터에 임대를 넣는 실험과 한 티켓으로 묶기 (D-041 V2, 별도)

조인 범위:

| 대상 | 조인 | 비고 |
|------|------|------|
| 아파트·연립·오피 | 정확 키, 실패 시 법정동+단지명 1:1 | 주거 집합만 |
| 단독다가구 | 안 함 | 임대 화면에만 |
| 분양권 | 안 함 | 매매에만 있음 |
| 임대만 있는 건물 | 매매 목록에 끼워 넣지 않음 | 임대 화면에 남김 |
| 매칭 실패 · 보조 1:n | “조인 없음” 명시 | 없는 것처럼 숨기지 않음 |

매칭은 **저장된 `building_key`** 만 정확 층으로 쓴다. 임대 SQL fallback 키(지번·도로, 단지명 없음)는 매매 이름 키와 다르므로 쓰지 않는다. 단지명 표기 차이로 해시가 갈라질 수 있어 보조 층이 있다.

매매 상세(같은 창)에 붙일 임대 패널: 전세/반전세/월세 원값 + 적용 r + 환산 P50 + n. 창 정합은 임대와 동일(3↔3, 5↔5, 7↔7). 임대 상세에 매매 패널을 다는 것은 같은 조인의 반대 방향이며 필수는 아님.

구현 순서:

1. 유형별 매칭률 리포트 (정확 / 보조 1:1 / 모호 / 매매만 / 임대만). 패널 게이트는 숫자 본 뒤
2. 매핑 테이블 후, 게이트 통과 유형만 매매 건물 상세에 임대 패널
3. 수익성(환산전세/매매 등)은 커버리지 통과 후에만 회귀 **후보 변수**. 기본 통계의 대표값이 되지 않게 함. Twin V2가 아님
...
- 적재: `py pipeline/rent/import_sangkwon.py` (폴더에서 **mtime 최신 xlsx** 1개. 분기 공표 파일이 과거 분기를 포함)
- 화면: 주거 목록과 섞지 않음. 「통계분석」 아래 **상권분석** 모달. 범위는 선택한 동의 시군구(구)와 교차하는 상권. 본문 지도에는 상권 없음. 모달 지도에 시군구 경계+상권 윤곽.
- 기본표 = **최신 분기 기준 4분기 1년 롤링** (예: 2025.3Q~2026.2Q). 임대료·층별임대료는 **월단가 평균×12**(만원/㎡·년). 순영업소득 금액은 **4분기 합**(만원/㎡·년). 소득·자본·투자수익률은 **4분기 복리**. 구성비·공실·전환율·지수는 분기 평균. 동수·층수·면적은 창의 마지막 분기. 4분기 없으면 금액·수익률은 빈칸.
- 추세: 2019년~ **달력 연간** 꺾은선. 주거 3/5/7 창과 묶지 않음.
- **분기 갱신:** 부동산원 새 분기 xlsx를 `임대시장/B.상업용/`에 두고 import 후 `rent_stats` Promote. 월간 사이클과 별개이나 체크리스트에 포함.
