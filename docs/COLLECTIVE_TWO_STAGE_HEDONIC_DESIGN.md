...
> **적용 범위:** 집합 주거의 `apartment`·`rowhouse`·`officetel`. 사용자가 둘 이상 유형을 선택하면 유형 더미를 포함한 통합 지역회귀로 유형 간 가격수준 차이를 설명한다. 비주거는 별도 cluster 분석 단위다.
...
### 0.3 지역회귀 개별공시지가 확장 계획

집합 주거 지역회귀는 현재 단지 속성(세대수·최고층·연식·주차·구조·시공사·유형)을
설명변수로 사용한다. 다음 실험에서는 **기본통계표의 대표 필지 1개에 연결한 최신
개별공시지가**를 선택형 보조변수로 추가한다. 종속변수의 선형/log 선택과 공시지가의
변환은 별개로 취급한다.

#### 범위와 1차 가설

- 종속변수는 기존처럼 `단가` 또는 `log(단가)` 중 사용자가 선택한다.
- 공시지가는 우선 원값으로 투입한다. UI에 `log(개별공시지가)` 선택지를 추가하지 않는다.
- 공시지가는 단지 전체 필지의 복잡한 평균이 아니라, 기본통계표의 대표 필지 1개의
  `㎡당 개별공시지가`를 사용한다.
- 최신 공시지가를 3·5·7년 평균 매매단가의 현재 토지·입지 수준 대리변수로 해석한다.
  인과효과나 거래연도별 예측변수로 해석하지 않는다.
- 아파트·연립·오피스텔에서 동일한 선택형 변수로 사용할 수 있어야 한다.

#### 구현 순서

1. 공식 원천과 기준연도를 확인하고 대표 필지 PNU 연결률을 측정한다.
2. `PNU·공시지가 기준연도·공시일·원천 버전`을 보존하는 별도 적재 구조를 만든다.
3. 단지-대표필지 연결과 공시지가 출처·기준연도를 단지 파생값으로 만든다.
4. 지역회귀 변수에 `assessed_land_price` 체크박스 하나를 추가한다.
5. 공시지가 결측은 보간하지 않고 표본 깔때기에 `대표 필지 공시지가 결측`으로 표시한다.
6. 기존 식과 `기존 식 + 원값 공시지가`를 동일 조건에서 비교한다.

#### 검증 기준

- 연결률과 결측 단지의 유형·지역 편중
- 동일 표본 기준 Adj. R² 변화
- holdout MAPE 변화
- 공시지가 계수의 방향·유의성·지역별 안정성
- 잔차 개선과 표본 감소율

공시지가가 개선을 보이지 않으면 기본식에 포함하지 않는다. 개선이 있더라도
사용자가 켠 경우에만 적합하는 선택형 블록으로 유지한다. `log(개별공시지가)`는
여러 지역 실험에서 원값보다 일관되게 우수할 때 별도 후보로 검토한다.

#### 적재 구현 (2026-08-25)

- 테이블은 `db/069_collective_building_assessed_land_price.sql`로 만들고,
  `pipeline/collective/import_assessed_land_price.py`가 기본적으로
  `raw/raw addition/개별공시지가(브이월드)` 아래의 `AL_D151` CSV를 읽는다.
- 원천 PNU가 사용하는 광주(`29`)·전남(`46`) 구 법정동코드는 `region_codes`의
  법정동명 3단계(시군구·읍면동·리) 1:1 매칭으로 현재 통합코드(`12`)로 변환한 뒤
