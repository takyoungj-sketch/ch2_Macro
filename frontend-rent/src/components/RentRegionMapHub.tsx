...
import { sangkwonHitsForAdmin } from "../utils/sangkwonIntersect";
...
type Props = {
  ...
  /** REB 상권 구획 (WGS84) */
  sangkwonFc?: GeoJSON.FeatureCollection | null;
  selectedSangkwon?: string | null;
  onSangkwonOverlap?: (names: { sec_nm: string; sido: string; buld_nm: string; overlapScore: number }[]) => void;
  onSelectSangkwon?: (name: string) => void;
};
...
function bboxOverlapScore(a: GeoJSON.Feature, b: GeoJSON.Feature): number {
  try {
...
export default function CollectiveRegionMapHub({
  ...
  const sangkwonHits = useMemo(() => {
    const selectedAdmin = (fillGeoJson?.features ?? []).filter(
      (f) => Number((f.properties as { ch2_selected?: number } | null)?.ch2_selected) === 1,
    );
    return sangkwonHitsForAdmin(selectedAdmin, sangkwonFc);
  }, [fillGeoJson, sangkwonFc]);

  const sangkwonDrawFc = useMemo((): GeoJSON.FeatureCollection | null => {
    if (!sangkwonFc || sangkwonHits.length === 0) return null;
    const names = new Set(sangkwonHits.map((h) => h.sec_nm));
    const features = sangkwonFc.features
      .filter((f) => names.has(String(f.properties?.sec_nm ?? "")))
      .map((f) => ({
        ...f,
        properties: {
          ...f.properties,
          ch2_sk_selected: String(f.properties?.sec_nm ?? "") === selectedSangkwon ? 1 : 0,
        },
      }));
    return { type: "FeatureCollection", features };
  }, [sangkwonFc, sangkwonHits, selectedSangkwon]);

  const lastSangkwonKeyRef = useRef("");
  useEffect(() => {
    const key = sangkwonHits.map((h) => h.sec_nm).join("|");
    if (key === lastSangkwonKeyRef.current) return;
    lastSangkwonKeyRef.current = key;
    onSangkwonOverlap?.(sangkwonHits);
  }, [sangkwonHits, onSangkwonOverlap]);
  ...
  const handleMapClick = useCallback(
    (evt: MapLayerMouseEvent) => {
      const skHit = evt.features?.find((f) => f.layer?.id === "sangkwon-fill");
      const skName = String(skHit?.properties?.sec_nm ?? "").trim();
      if (skName && onSelectSangkwon) {
        onSelectSangkwon(skName);
        return;
      }
      const fromEvent = evt.features?.[0]
        ? ({
    ...
    [offerAddRegionAt, onSelectSangkwon],
  );
      ...
      >
        {showSetupHint ? (
          <div className="absolute inset-0 flex items-center justify-center p-6 text-center text-sm text-slate-600 bg-slate-100 z-10">
            <div className="space-y-2 max-w-md">
              ...
              <p className="text-xs text-slate-500">
                `frontend-rent/.env` 에 <code className="text-[11px]">VITE_VWORLD_API_KEY</code> 를
                설정하세요.
              </p>
            </div>
          </div>
          ...
          <MapGL
            ...
            interactiveLayerIds={
              [
                ...(fillGeoJson ? ["region-fill", "region-outline"] : []),
                ...(sangkwonDrawFc ? ["sangkwon-fill"] : []),
              ]
            }
          >
            ...
            {sangkwonDrawFc ? (
              <Source id="sangkwon-fill" type="geojson" data={sangkwonDrawFc}>
                <Layer
                  id="sangkwon-fill"
                  type="fill"
                  paint={{
