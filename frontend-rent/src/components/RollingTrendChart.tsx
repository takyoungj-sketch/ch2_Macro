...
/** 12개월 롤링 버킷 추이 — x축 YY.MM~YY.MM. 집합 매매 창과 동일 형식. */
export default function RollingTrendChart({
  ...
  return (
    <div className="w-full overflow-x-auto" role="img" aria-label={`롤링 구간별 ${priceLabel} 및 거래 건수 추이`}>
      ...
      >
        ...
        {sorted.map((r, i) => (
          <circle key={`c-${r.bucket_index}`} cx={xAt(i)} cy={yCount(r.count)} r={3.5} fill="#fff" stroke={COUNT_MARKER_STROKE} strokeWidth={2} />
        ))}
        {sorted.map((r, i) => (
          <text
            key={`cl-${r.bucket_index}`}
            x={xAt(i)}
        ...
        {meanLineRows.length > 0 && (
          <>
            ...
            {meanLineRows.map((r) => {
              ...
              return (
                <circle
                  key={`m-${r.bucket_index}`}
                  cx={xAt(idx)}
                  cy={yMean(Number(r.mean))}
                  r={3.5}
                  fill="#fff"
                  stroke="#2563eb"
                  strokeWidth={2}
