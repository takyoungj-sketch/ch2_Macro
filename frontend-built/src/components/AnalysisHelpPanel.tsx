...
export default function AnalysisHelpPanel({
  ...
  return (
    <div className={clsx("relative", className)}>
      ...
      {open && (
        <div className="absolute right-0 top-full z-20 mt-1 w-[min(22rem,calc(100vw-2rem))] rounded-lg border border-indigo-100 bg-white shadow-lg p-3 space-y-3 max-h-[min(420px,55vh)] overflow-y-auto">
          ...
          {explain.formula && (
            <Section title="공식">
              <p className="font-mono text-[10px] bg-slate-50 border border-slate-100 rounded px-2 py-1.5 whitespace-pre-wrap">
                {explain.formula}
              </p>
            </Section>
          )}
            ...
            <Section title="자주 묻는 질문">
              <div className="space-y-1">
                {explain.presets.map((p) => (
                  <div key={p.id} className="rounded border border-slate-100 bg-slate-50/80 overflow-hidden">
                    <button
                      type="button"
                    ...
                    {activePreset === p.id && p.answer && (
                      <p className="px-2 pb-2 text-[11px] text-slate-600 border-t border-slate-100">{p.answer}</p>
                    )}
                  </div>
                ))}
              </div>
          ...
          <p className="text-[9px] text-slate-400 pt-1 border-t border-slate-100">
            AI 어시스턴트 버튼으로 이 화면 수치·spec 기반 질문도 가능합니다.
          </p>
        </div>
      )}
    </div>
