...
""",
    "insight": """
Macro Insight:
- 공개 FAQ. 전국 월 시계열 실험 기록(1번: 금리·M2와 거래 건수·액).
- 숫자는 화면 Bundle(공개 /api/insight/01)만. 랩 API를 부르지 않음.
- 상관계수 r는 표의 칸. 문장에 없는 r를 만들지 않음.
- 바람직: 같이 움직였나를 쉬운 말로. 물음표 → AI 추가 질문.
- 한계: 인과·예측·결론이 아님. 같은 질문에 실험을 더하면 본문이 늘어날 수 있음. 시군구 r 없음.
""",
}
