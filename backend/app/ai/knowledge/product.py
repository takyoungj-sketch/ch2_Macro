...
- insight(Macro Insight): 게이트웨이 FAQ. 실험 기록·기능 예시. 분석 앱이 아님. 결론을 내리지 않음
"""
...
- 보정 범위(D-074): 점추정·95% 평균 추정범위에는 적용, 예측구간에는 미적용(분위수는 단조변환 보존). 따라서 추정값이 예측구간의 가운데가 아닌 것은 정상. 보정 크기는 로그 잔차 σ에 따라 exp(σ²/2) — σ=0.4면 +8%, σ=0.6면 +20%
- 한계: smearing은 전역 상수 하나라 등분산을 가정. 잔차 분산이 규모에 따라 커지면 큰 물건에서 보정이 부족할 수 있음
- **시점 보정은 하지 않는다**(D-075 보류). 회귀는 창 기간(기본 5년)의 거래를 시점 통제 없이 함께 적합하므로, 추정값은 특정 연도 시세가 아니라 **그 창의 명목 가격 수준**을 가리킨다. 「현재 시세」「지금 가격」이라고 단정하지 말 것. 사용자가 시점을 물으면 창 기간을 밝히고, 최근 시세만 보려면 조회 연도를 좁히라고 안내한다
- 보정 계층은 구현돼 있으나 꺼 둔 상태다(요청 time_adjust). 5년 창 실측에서 adj R²·MAPE 개선 근거가 없는데 가격 수준이 20~37% 움직였고 그 이동을 CV로 검증할 수단이 없어 제품에 넣지 않았다. **켜져 있다고 설명하지 말 것**
- 창이 길수록 시점 미통제의 대가가 커진다. 8년 구간 실험에서는 강남 상가 연식 계수가 절반으로 축소돼 「연식이 거의 상관없다」처럼 보였다. 5년에서는 그 왜곡이 작다. 계수 해석 시 창 기간을 함께 밝히면 좋다
- 기초 정의(Adj R²·VIF·p 등)는 UI 지표 옆 ? 팝업 — AI는 이번 결과 해석에 집중
"""

RESIDUAL_DIAGNOSTICS = """
잔차 진단 (초점 모형 아래 「이 식이 어디서 틀리나」 카드, P5):
- Adj R²·MAPE는 「평균적으로 얼마나」만 말한다. 이 카드는 **어디서** 틀리는지를 본다 — 집단별 치우침, 금액대별 오차 폭, 결과를 끌고 있는 거래.
- 오차% = (실제 − 예측) ÷ 실제 × 100. **양수 = 모형이 싸게 봤다**(실제가 예측보다 높다). 예측은 화면 상단 MAPE와 같은 방식(log 모형이면 Duan 보정 포함)이라 집단별 MAPE를 건수로 가중평균하면 상단 MAPE가 된다.
- **전체 중위 오차가 음수인 것은 흔하고 결함이 아니다.** 금액이 작은 거래에서 백분율 오차가 한쪽으로만 크게 벌어진다(−300%는 가능하지만 +300%는 불가). 그래서 평균 편향은 중위보다 늘 음수 쪽이다. 운영 실측에서 강남 평균 −12.6% / 중위 −6.0%. 「모형이 전반적으로 과대평가한다」고 설명하지 말 것. 치우침은 중위로 읽는다.
- 집단별로는 **전체 대비(excess, %p)**를 읽는다. 그 집단 중위 − 나머지 표본 중위이므로 표본 공통 치우침이 빠진 값이다. 절대 중위는 참고값이다.
- ● 표시는 **그 집단 vs 나머지 표본 Mann–Whitney p<0.05**다. 평균을 0과 비교하는 방식이 아니다(그 방식으로는 공통 치우침 때문에 전 집단이 유의하게 찍혔다). 표시가 없으면 나머지와 다르게 틀린다고 볼 근거가 없다는 뜻이며 구조적 편향으로 읽지 않는다.
- 10건 미만 집단은 표에 넣지 않고 「표에서 뺀 N건」으로만 센다.
- 축별로 읽는 법:
  · **지역**이 계속 치우치면 지역 더미 추가 또는 Twin(상위지역 붙임)을 검토할 근거. 잔차가 남는다는 증거이므로 공간모형 논의의 진입 조건도 된다.
  · **연식 구간**이 계단처럼 갈라지면 연식을 직선으로 본 가정이 부족하다는 신호(연식² · 구간 더미 · spline 검토).
  · **계약연도**가 치우치면 조회 기간 중 시장이 움직였다는 뜻. 시점 보정을 켜지 않았으므로(D-075) 이 축은 그 비용을 드러내는 자리다. 기간을 좁히면 줄어든다.
  · **용도지역·건축물용도**가 치우치면 그 더미를 변수에 넣었는지 확인. 이미 넣었다면 유의 집단이 안 나오는 게 정상이다.
- 금액대별: 예측금액 5분위의 **오차 산포**를 본다. 형식 검정(Breusch–Pagan)도 함께 싣지만 **분위 산포가 우선**이다. BP는 금액대가 아니라 설계행렬 전체에 반응하고 n이 크면 사소한 위반에도 기각한다. 운영에서 강남은 BP p=0.000인데 산포는 42~46%로 평평했고, 수성은 p=0.109인데 산포가 83%→43%였다. **BP만 보고 「금액대에 따라 오차가 다르다」고 말하지 말 것.**
- 결과를 끌고 있는 거래: Cook 거리 상위 5건. 오차 크기만이 아니라 「빼면 계수가 얼마나 움직이나」를 함께 보는 지표다. 아래 계수 변화는 **표준오차 배수**로 잰다(계수 스케일이 다르고 0 근처는 백분율이 폭발). 1 SE를 넘으면 소수 거래가 결과를 끌고 있다는 뜻.
- **in-sample 잔차다.** 같은 자료로 적합하고 같은 자료로 재므로 실제 예측오차보다 낙관적이다. 예측력을 물으면 모형 탐색의 CV-MAPE로 넘긴다. 이 카드의 숫자를 예측 정확도로 인용하지 말 것.
- 초점 모형에만 붙는다. 상위지역 재적합에는 없다.
"""

NESTED_ADMIN_SCOPE = """
복합 회귀의 행정 계층 (History 실행 순서와 다름):
- 초점(하위): 지금 고른 단위. 대개 읍·면·동. 기본 회귀·예측의 primary.
- 상위지역: 직계 상위 시·군·구 등 comparisons. 화면 이름은 「상위지역 재적합」이다.
- **같은 모형의 확대 적용이 아니다.** 상위 단위 표본에서 회귀를 처음부터 다시 적합한 별개의 모형이다. 「같은 식을 넓혔다」「같은 모형을 확대했다」고 설명하지 말 것.
- 설명 순서를 지킨다: ① 두 모형이 어떻게 다른지(표본·변수 구성) → ② 공통 변수의 계수 방향 → ③ 그 다음에만 수치. 숫자부터 비교하지 않는다.
- 모형이 달라지는 두 가지 이유:
  (a) 표본이 중첩된다 — 상위 표본이 초점 거래를 포함한다. 독립된 두 표본의 비교가 아니다.
  (b) 시군구·구 단계에서는 **지역 더미를 넣지 않는다** — 그 단위 자체가 표본 경계라 비교할 다른 지역이 표본에 없어 지역 간 차이를 담을 열이 만들어지지 않는다. 정책이며 누락·버그가 아니다. 초점에 지역 더미가 있었다면 상위 모형은 그 변수가 빠진 다른 사양이다. 결과의 warning에 「지역 더미 제외 — 이 단계가 표본 경계」로 실려 온다.
- **Adj R²·MAPE를 단계 간 우열로 판정하지 말 것.** 표본 크기와 변수 구성이 동시에 다르면 두 지표는 비교 가능한 값이 아니다. 「상위가 더 설명력이 높다/낮다」는 결론을 내지 않는다. 각 수치는 그 단계 표본 안에서만 유효하다고 말한다.
- 읽을 수 있는 것: 공통 연속변수(연면적·대지면적·연식)의 **부호가 유지되는지**. 유지되면 그 패턴이 더 넓은 규모에서도 읽힌다는 참고(유사 지역 범위 규모의 간접 힌트)다. 더미는 기준 범주가 단계마다 달라질 수 있어 부호를 견주지 않는다. Twin 채택·적정가가 아니다.
- 의도: 하위 단위에서 표본·변수가 얇을 때 범위를 넓혀 같은 골격의 회귀를 다시 시도해 패턴의 재현성을 본다.
...
RECOMMEND_LOGIC = """
모형 추천·Twin:
- 복합 Macro 탐색: ① CV-MAPE 대표 예측모형 → ② Local 기준선 → ③ Twin1(같은 식에 쌍둥이 1위만 보탬) → ④ Twin2(확인용 재탐색) → ⑤ Local vs Twin1 vs Twin2 숫자 비교.
- Twin 후보는 지역프로필 벡터(algo 21). 유사지역 **목록**만 보려면 /profile/ Twin 카드.
- 복합 ③ Twin1 · ④ Twin2 실험이 돌면 결과는 지금 화면 Bundle facts.stage2 다 (ran, twin_experiments, Local vs Twin n·탐색/확인 CV, 채택 권고, inspect_pool, research). 있으면 그 숫자를 인용한다. 「이 패널에서는 Twin을 못 본다」고 하지 말고 Profile로 보내지 않는다.
- Twin1은 Local 식·척도를 고정하고 쌍둥이 1위만 보태며, Local에 지역 더미가 있든 없든 region_leaf를 넣는다. CV 채택과 무관하게 그 재적합 예측을 ③에서 본다. Local 식은 ②에 그대로 둔다.
- Twin 실험2는 Local + Twin 1위 표본에서 예측형(CV-MAPE) 식을 다시 고르는 출시 전 확인용이다. 설명형 AIC는 쓰지 않는다. 결과는 ④. 기본 통계 식은 덮지 않는다.
- 채택은 확인 CV 개선 + 주요 계수 방향. n만으로 채택하지 않는다. 후보는 거래가격이 아니라 구성·체급 (D-066).
- 탐색 CV↔확인 CV (D-074): 마지막 연도는 후보 선택에 일절 쓰지 않는다. 탐색 CV는 그 앞의 연도들, 확인 CV는 마지막 연도만. 그래도 탐색 CV는 수백 후보 중 최소값이라 낙관적이므로 두 값의 격차를 안정성으로 읽는다.
- Twin 판정은 양쪽 탐색 CV가 있을 때만 한다. 얇은 표본은 마지막 연도를 떼면 fold를 못 만들 수 있고, 그때는 「판정하지 않고 Local 유지」로 나온다. AIC로 대신 판정하지 않는다 — 표본이 다르면 AIC는 비교 불가(로그우도가 n에 따라 커짐).
- CV-MAPE는 예측을 자르지 않은 값이다. 대신 「범위 밖 예측 비율」(학습 가격의 ×0.1~×10 밖)과 중위 오차를 안정성 진단으로 따로 본다. 평균과 중위가 크게 벌어지면 소수 거래가 평균을 끌고 있다는 뜻이다. 진단 비율을 성능으로 인용하지 않는다.
- 공식 지표는 **평균** APE다. 중위로 바꾸지 않았다(D-074, 전수 31후보 실증). 중위 기준으로 고르면 예측 다발은 촘촘하고 꼬리가 두꺼운 희소 모형이 1위가 되는데(강남구에서 중위 1위 = 평균 17위인 2변수 모형), 그 꼬리가 사용자가 설명을 요구하는 거래다. 중위는 「대다수 거래는 이 정도, 평균은 꼬리가 끌어올린 값」으로 읽는 진단이다.
- 만족도 등급은 **탐색·확인 CV 중 나쁜 쪽**으로 준다. 확인이 더 좋게 나와도 등급을 올려 주지 않는다(fold 1개라 그 해가 쉬웠을 뿐일 수 있음). 확인이 등급을 정했으면 화면에 「(마지막 연도 기준)」이 붙는다. 한쪽만 있으면 그 값으로 준다.
- Twin이 이겨도 기본 통계 식은 덮지 않는다. 같은 골격의 재적합 식·예측은 이 창 ③ Twin1 · ④ Twin2. 유사도 ≠ 자동 채택. Twin Lab ≠ 제품 기본.
- 「기본 통계에 이 식 적용」(P6): 사용자가 확인한 뒤에만 ① 대표 예측모형의 변수·척도를 왼쪽 선택에 옮긴다. 통계분석을 자동 실행하지 않는다. 오른쪽 숫자는 「통계분석」을 다시 눌러야 갱신된다. Twin 식 적용은 지역 범위를 바꾸므로 연결하지 않았다. 탐색이 식을 몰래 덮었다고 설명하지 말 것.
"""
...
- 행정 계층: 초점(대개 읍면동)=하위 단위, 「상위지역 재적합」=직계 상위 시군구. 하위 n·변수가 얇을 때 같은 골격으로 상위 표본에서 **다시 적합**한다(같은 모형의 확대가 아님). 표본이 중첩되고 시군구·구에서는 지역 더미가 빠져 Adj R²·MAPE를 단계 간 우열로 읽을 수 없다. 공통 연속변수의 부호가 유지되면 더 넓은 규모의 참고(유사지역 범위의 간접 힌트). Twin 채택·적정가가 아님.
...
""",
    "insight": """
Macro Insight:
- 공개 FAQ. 1번 전국 월(금리·M2와 거래), 2번 시군구 유형 규모·단가, 3번 연립·다세대 건물 안 층×승강기, 4번 토지 칸 안 면적×㎡당 가격, 5번 연 거래액 vs GDP·M2·주식·유형 구성.
- 1·2번 숫자는 공개 /api/insight/01·02. 3·4·5번은 고정 스냅샷(copy). 랩 API를 부르지 않음.
- 상관계수 r·퍼센트는 표의 칸. 문장에 없는 숫자를 만들지 않음. 3번의 13.5와 26을 더하지 않음. 4번의 약 −20%를 전국 할인율·회귀 계수로 쓰지 않음. 5번의 거래액/GDP를 GDP 기여도로 쓰지 않음.
- 바람직: 질문을 쉬운 말로. 물음표 → AI 추가 질문.
- 한계: 인과·예측·결론이 아님. 같은 질문에 실험을 더하면 본문이 늘어날 수 있음.
""",
}
...
""",
    "insight": """
Macro Insight에서 하지 않는 것:
- 분석 앱(토지·복합·집합·임대·프로필)이 아님. 이 지역 시세를 구하지 않음.
- 금리가 거래를 줄였다·동조가 검증됐다·결론이다 라고 쓰지 않음.
- 표에 없는 r·시군구 상관·연도 표를 만들지 않음.
- 랩 /lab/ 숫자를 몰래 가져오지 않음.
- 앞으로 금리·거래가 어떻게 된다는 전망 금지.
- 3번에서 승강기를 설치하면 13% 오른다고 쓰지 않음. 13.5와 26을 더하지 않음. 아파트 층 지수와 같다고 쓰지 않음.
- 4번에서 약 −20%를 전국 광평 할인율이라고 쓰지 않음. 대지는 클수록 비싸다고 쓰지 않음. 토지 회귀 면적 계수와 같다고 쓰지 않음.
- 5번에서 거래액/GDP를 GDP 기여도·건설업 비중이라고 쓰지 않음. 시중 돈이 부동산으로 갔다, 주식이 대체됐다고 쓰지 않음. 1번 월 상관과 섞지 않음.
""",
}
...
def format_nested_scope_answer(
    *,
    ...
    lines = [
        ...
        "행정 계층을 넓혀 회귀를 다시 시도하는 설계입니다.",
        "",
        "**하위(초점)** 는 지금 고른 단위입니다. 대개 읍·면·동이고, 기본 회귀·예측의 primary입니다.",
        "**상위** 는 직계 상위 시·군·구입니다. 화면 「상위지역 재적합」이 그 표본에서 회귀를 다시 적합합니다.",
        "",
        "같은 모형을 넓힌 것이 아니라 **별개의 재적합**입니다. 표본이 중첩되고(상위가 초점 거래를 포함), "
        "시군구·구 단계에서는 그 단위 자체가 표본 경계여서 지역 더미가 빠집니다. "
        "그래서 Adj R²·MAPE를 단계 간 우열로 읽을 수 없습니다.",
        "",
        "의도는 두 가지입니다. 하위 단위에서 표본·변수가 얇을 때 범위를 넓혀 회귀를 시도할 수 있게 하는 것, "
        "그리고 공통 연속변수의 계수가 윗단계에서도 같은 방향이면 그 패턴이 더 넓은 규모에서도 읽힌다는 **참고**(유사 지역 범위 규모의 간접 힌트)를 얻는 것입니다. "
        "Twin 채택이나 적정가가 아닙니다.",
        "",
        ...
        if comparisons:
            ...
            if want_compare:
                ...
                if n_p is not None and n_u is not None:
                    lines.append(
                        f"  상위 n={n_u} 안에 초점 n={n_p} 이 포함됩니다(중첩). "
                        "윗단계 표본이 더 많은 것은 하위 단위의 얇은 표본을 보완하려는 시도이며, "
                        "두 표본이 독립이 아니므로 적합도 수치를 우열로 견주지 않습니다."
                    )
        else:
            lines.append(
                "- 직계 상위 숫자(comparisons)는 아직 Bundle에 없습니다. "
                "「상위지역 재적합」을 열면 윗단계에서 다시 적합한 결과를 볼 수 있습니다."
            )
    elif want_compare:
        lines.append("")
        lines.append(
            "지금 화면에 초점 vs 직계 상위 숫자가 없습니다. "
            "「상위지역 재적합」을 연 뒤 같은 질문을 하시면 그 값을 인용합니다."
        )
...
읍면동 표본이 얇으면 「상위지역 재적합」에서 직계 상위(시군구) 표본에 회귀를 다시 적합한다 — 같은 모형의 확대가 아니다. History 1차·2차(실행 순서)와 혼동하지 않는다.
...
def format_howto_answer(app: str, message: str) -> str:
    ...
    if app == "insight":
        return (
            "Macro Insight는 질문 목록입니다. 1번을 누르면 전국 월 기준으로 "
            "금리·시중 돈과 거래가 같이 움직였는지를 봅니다. "
            "결론이 아니며, 헤더의 AI 어시스턴트에 추가 질문할 수 있습니다."
        )
    return (
...
FUNCTION_CARDS: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "name": "복합 상위지역 재적합",
        "apps": ("built",),
        "purpose": "초점(읍면동) 표본이 얇을 때 직계 상위(시군구) 표본에서 회귀를 다시 적합해, 같은 패턴이 더 넓은 규모에서도 읽히는지 본다",
        "good_questions": [
            "1차와 2차가 의미하는 바는?",
            "상위와 하위 행정구역을 왜 같이 보나",
            "읍면동 n이 부족하면?",
        ],
        "strengths": ["같은 골격으로 상위 표본에서 재적합", "공통 연속변수의 계수 방향이 유지되면 규모 인사이트의 간접 힌트"],
        "cautions": [
            "같은 모형의 확대 적용이 아니라 별개 재적합 — 모형 차이를 먼저 설명할 것",
            "표본이 중첩되고 시군구·구에서는 지역 더미가 빠져 Adj R²·MAPE를 단계 간 우열로 읽을 수 없음",
    ...
    },
    {
        ...
        "cautions": [
            "복합 Macro ③ Twin 실험이 Bundle facts.stage2에 있으면 그 n·탐색/확인 CV·채택 권고를 인용한다. Profile로 보내지 않음",
...
def product_knowledge_pack(*, app: str = "built", panel: str = "") -> str:
    ...
    if key == "built":
        ...
        parts.append(RESIDUAL_DIAGNOSTICS.strip())
        ...
        parts.append(
            "insight: Macro Insight FAQ. 통계 기초만 있는 사람에게 쉬운 말. "
            "논문 문체·인과 단정·결론 금지. 화면 표의 r만 인용. 재계산 금지. "
            "이 기록은 열려 있다. 추가 실험이 붙을 수 있다."
        )
