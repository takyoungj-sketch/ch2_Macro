...
# match_tier → 라벨·신뢰도·회귀 사용 가능 여부 (설계 §3.1: A·B·C만 hard 표본)
TIER_META: dict[str, dict[str, Any]] = {
    ...
    "D": {"label": "지번 다중후보(판정 보류)", "reliability": "판정 보류", "usable": False},
    "E": {"label": "단지명 부분일치(단일후보)", "reliability": "낮음", "usable": False},
    "F": {"label": "단지명 부분일치(다중후보)", "reliability": "판정 보류", "usable": False},
...
def source_label(snapshot_ym: str | None) -> str:
...
def _fetch_row(
    ...
    row = conn.execute(
        text(
            f"""
            SELECT {_ATTR_COLUMNS}, {select_name}
...
def _unmatched_payload(
    *,
    ...
    return {
        ...
        "source_label": source_label(snapshot_ym),
...
def fetch_danji_attributes(
    ...
    *,
    ...
    # 단지 속성은 K-apt 단지 1건과 연결됐을 때만 의미가 있다. tier D·F는 후보가
    # 여러 개라 danji_code가 비어 있으므로 미매칭으로 다룬다.
    matched = row["danji_code"] is not None
